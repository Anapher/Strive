...
{
    public class ChatRepository : IChatRepository, IRedisRepo
    {
        ...
        {
            return RedisKeyBuilder.ForProperty(PROP_KEY).ForConference(conferenceId).ForSecondary(channel).ToString();
        }

        private static string GetChannelSetKey(string conferenceId)
        {
            return RedisKeyBuilder.ForProperty(PROP_CHANNELS_KEY).ForConference(conferenceId).ToString();
        }

        private static string GetTypingSetKey(string conferenceId, string channel)
        {
            return RedisKeyBuilder.ForProperty(PROP_TYPING).ForConference(conferenceId).ForSecondary(channel)
                .ToString();
        }
    }
}
