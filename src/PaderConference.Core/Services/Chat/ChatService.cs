...
namespace PaderConference.Core.Services.Chat
{
    public class ChatService : ConferenceService
    {
            ...
            using (_logger.BeginScope(message.GetScopeData()))
            {
                ...
                if (messageDto.Mode != null)
                    ...
                    else if (messageDto.Mode is SendPrivately sendPrivately)
                    {
                        ...
                        if (!_connectionMapping.ConnectionsR.Any(
                            x => x.Key.ParticipantId == sendPrivately.ToParticipant))
                        {
                ...
                var filter = FilterFactory.CreateFilter(messageDto.Mode, message.ConnectionId);
                ...
                await chatMessage.Filter.SendAsync(_signalMessenger, _conferenceId,
        ...
        public ValueTask<IReadOnlyList<ChatMessageDto>> RequestAllMessages(IServiceMessage message)
        {
            ...
            try
            {
                messages = _messages.Where(x => x.Filter.ShowMessageTo(message.ConnectionId)).ToList(); // copy
            }
            finally
            {
