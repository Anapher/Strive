...
namespace PaderConference.IntegrationTests.Controllers
{
    ...
    public class SfuTest : ServiceIntegrationTest
    {
        ...
        {
            // arrange
            var response = await Client.GetAsync($"/v1/sfu/123?apiKey={new SfuOptions().ApiKey}");
        ...
        public async Task Fetch_UserJoined_NotEmpty()
        {
            ...
            var response = await Client.GetAsync($"/v1/sfu/{conference.ConferenceId}?apiKey={new SfuOptions().ApiKey}");
